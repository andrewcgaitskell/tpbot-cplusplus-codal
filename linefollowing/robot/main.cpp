...
static const int   TRIM       = 2;     // constant bias to cancel a mechanical left-drift confirmed via diagnostic
                                        // testing (equal wheel speeds alone drifted left). Left gets +TRIM,
                                        // right gets -TRIM, independent of the P correction below.
static const int   MIN_SPEED  = -25;  // allows the inner wheel to reverse for a tighter pivot on sharp error
static const int   MAX_SPEED  = 100;
// Slew-rate limiting was tried here to smooth the motor "burst" on
// direction changes, but with only ~3.5mm margin either side of the
// line, ramping the correction in over multiple loops took too long -
// the robot drifted off before reaching full correction. Reverted:
// following the line correctly matters more than smoothing the motors,
// and the margin here doesn't leave room for both at once.
...
int main()
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            int leftSpeed  = clampSpeed(BASE_SPEED + TRIM - output);
            int rightSpeed = clampSpeed(BASE_SPEED - TRIM + output);
