...
struct __attribute__((packed)) RobotCommand
{
    ...
    int16_t value1;     // meaning depends on command, e.g. left wheel speed
    int16_t value2;     // meaning depends on command, e.g. right wheel speed
};
...
{
    CMD_SET_WHEELS = 1,
    // CMD_HEADLIGHT = 2,  // example of how to extend later
};
